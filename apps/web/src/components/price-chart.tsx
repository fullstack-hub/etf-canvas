...
import { XAxis, YAxis, Tooltip, AreaChart, Area, CartesianGrid, LineChart, Line, ReferenceLine, ResponsiveContainer } from 'recharts';
...
interface Props {
  data: ETFDailyPrice[];
  compact?: boolean;
  /** SafeChartContainer에서 전달받는 크기 (compact 모드) */
  chartWidth?: number;
  chartHeight?: number;
}

export function PriceChart({ data, compact, chartWidth, chartHeight }: Props) {
  ...
  if (compact) {
    ...
    return (
      <AreaChart width={w} height={h} data={chartData} margin={{ top: 4, right: 12, left: -4, bottom: 0 }}>
        <defs>
